...
public class FancyMessage implements JsonRepresentedObject, Cloneable, Iterable<MessagePart>, ConfigurationSerializable
{
    ...
    private void send(final CommandSender sender, final String jsonString) {
        ...
        try {
            ...
            Bukkit.getLogger().log(Level.WARNING, "Could not find class.", e6);
        }
    }
    ...
    private Object createChatPacket(final String json) throws IllegalArgumentException, IllegalAccessException, InstantiationException, InvocationTargetException, NoSuchMethodException, ClassNotFoundException {
        if (FancyMessage.nmsChatSerializerGsonInstance == null) {
            ...
            final String[] split = version.substring(1, version.length() - 1).split("_");
            final int minorVersion = Integer.parseInt(split[1]);
            final int revisionVersion = Integer.parseInt(split[2].substring(1));
            ...
            Field[] arrayOfField;
            for (int j = (arrayOfField = chatSerializerClazz.getDeclaredFields()).length, i = 0; i < j; ++i) {
                final Field declaredField = arrayOfField[i];
